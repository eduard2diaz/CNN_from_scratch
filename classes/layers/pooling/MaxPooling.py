...
class MaxPooling(Layer):
    ...
    def forward(self, mtx_tensor):  # mtx_tensor : (n_instances, n_rows, n_cols, n_filters)
        ...
        def maxPoolingSelection(mtx, output):
            """
            Funcion que calcula maxPooling para una matriz de la forma (n_rows, n_cols). Es decir, crea una submatriz
            compuesta por el mayor valor de cada uno de los "cuadrantes"
            :param mtx: matriz sobre la que calcular maxPooling
            ...
            for i in range(output.shape[0]):
                ...
                for j in range(output.shape[1]):
                    ...
                    output[i][j] = np.max(submatrix.reshape((-1, 1)))
                    mtx_j += self.strides
                mtx_i += self.strides

        for instance in range(mtx_instances):
            for channel in range(mtx_filters):
                maxPoolingSelection(mtx_tensor[instance, :, :, channel], output[instance, :, :, channel])
    ...
    def backward(self, output_gradient, learning_rate):
        ...
        :param output_gradient: derivada de la salida de la capa.
        """
        ...
        def maxPoolingSelectionRevert(last_input_mtx, output_gradient):
            def create_mask_from_window(x):
                """
                Returns: Array of the same shape as window, contains a True at the position corresponding to
                the max entry of x.
                param x: matriz
                """
                mask = x == np.max(x)
            ...
            for i in range(output_gradient.shape[0]):
                ...
                for j in range(output_gradient.shape[1]):
                    submatrix = last_input_mtx[mtx_i: mtx_i + self.pooling_shape[0],
                                mtx_j: mtx_j + self.pooling_shape[1]]

                    gradiente = output_gradient[i][j]  # obtengo el gradiente
                    mask = create_mask_from_window(
                        submatrix)  # Obtengo la mascara que señala el maximo valor de submatrix
                    # Actualizo la matriz de gradientes de entrada
                    padding_out[mtx_i: mtx_i + self.pooling_shape[0],
                    mtx_j: mtx_j + self.pooling_shape[1]] += np.multiply(mask, gradiente)
                    mtx_j += self.strides
                mtx_i += self.strides
        ...
        for instance in range(mtx_instances):
            for filter_index in range(mtx_filters):
                output[instance, :, :, filter_index] = maxPoolingSelectionRevert(self.last_input[instance, :, :, filter_index],
                                                                        output_gradient[instance, :, :, filter_index])

        return output

    def __str__(self):
        return f"MaxPoolingLayer(padding: {self.padding}, strides: {self.strides})"